...
import VehicleGrid from '../components/VehicleGrid';
import { vehicles } from '../utils/vehiclesData';
import { FaCar, FaUsers, FaClock, FaShieldAlt, FaStar, FaSearch } from 'react-icons/fa';

export default function LuxuryCar({ favorites, toggleFavorite }) {
  const [isVisible, setIsVisible] = useState({});
  const [filterType, setFilterType] = useState('all');
  const [searchTerm, setSearchTerm] = useState('');
  const [priceType, setPriceType] = useState('metro');
  const observerRefs = useRef([]);
  ...
  const carTypes = [
    ...
    { id: 'suv', label: 'SUVs', icon: FaCar },
    { id: 'luxury', label: 'Luxury', icon: FaStar },
    { id: 'van', label: 'Commuter Vans', icon: FaCar }
  ];
      ...
      <section className="relative py-24 bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 overflow-hidden">
        ...
        <div className="container mx-auto px-6 lg:px-12 relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-blue-100 leading-relaxed mb-8">
              Experience the pinnacle of automotive luxury with our meticulously maintained fleet.
            </p>
      ...
      <section className="py-8 bg-white border-b border-gray-200 sticky top-0 z-40 shadow-sm">
        <div className="container mx-auto px-6 lg:px-12">
          ...
          <div className="flex gap-3 justify-center mt-6">
            {["metro", "tier2", "luxury"].map((type) => (
              <button
                key={type}
                onClick={() => setPriceType(type)}
                className={`px-4 py-2 rounded-lg font-medium capitalize ${
                  priceType === type ? "bg-blue-600 text-white" : "bg-gray-200"
                }`}
              >
                {type === "metro" && "Metro Cities"}
                {type === "tier2" && "Tier 2 Cities"}
                {type === "luxury" && "Luxury Pricing"}
              </button>
            ))}
          </div>
